import mongoose from "mongoose";
import dotenv from "dotenv";
import colors from "colors";
import users from "./data/users.js";
import WatchListData from "./data/watchlists.js";
import StockStorageData from "./data/storages.js";
import HistoryData from "./data/history.js";
import User from "./models/userModel.js";
import Watchlist from "./models/watchlistModel.js";
import Storage from "./models/storageModel.js";
import History from "./models/historyModel.js";
import connectDB from "./config/db.js"; 

dotenv.config();
connectDB();

const importData = async () => {
    try {
        await User.deleteMany();
        await Watchlist.deleteMany();
        await Storage.deleteMany();
        await History.deleteMany();

        const createdUsers = await User.insertMany(users);
        const adminUser = createdUsers[0]._id;
        const sampleWatchList = WatchListData.map((watchlist) => {
            return { ...watchlist, user: adminUser };
        })
        const sampleStorage = StockStorageData.map((storage) => {
            return { ...storage, user: adminUser };
        })
        const sampleHistory = HistoryData.map((history) => {
            return { ...history, user: adminUser };
        })
        const watchlist = await Watchlist.insertMany(sampleWatchList);
        const stockstorage = await Storage.insertMany(sampleStorage);
        const history = await History.insertMany(sampleHistory);
        console.log("Data Imported!".green.inverse);
        process.exit();
    } catch (error) {
        console.error(`${error}`.red.inverse);
        process.exit(1);
    }
}

const destroyData = async () => {
    try {
        await User.deleteMany();
        await Watchlist.deleteMany();
        await Storage.deleteMany();
        console.log("Data Destroyed!".red.inverse);
        process.exit();
    } catch (error) {
        console.error(`${error}`.red.inverse);
        process.exit(1);
    }
}

if (process.argv[2] === "-d") {
    destroyData();
} else {
    importData();
}
