...
import { mean, cov, multiply } from 'mathjs';
...
const Recommend = () => {
  ...
  const generateRecommendations = async () => {
    try {
      ...
      const covarianceMatrix = cov(returnsMatrix);
  ...
  const generatePortfolios = (stockData, covarianceMatrix) => {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      // 计算组合收益率和风险
      const portfolioReturn = stockData.reduce((acc, stock, index) => acc + (mean(stock.dailyReturns) * normalizedWeights[index]), 0);
      const variance = multiply(multiply(normalizedWeights, covarianceMatrix), normalizedWeights);
